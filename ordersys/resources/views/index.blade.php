						...
						<div class="order-form-v2__form-row js_order_form_paper_type" style="direction: ltr;">
							...
							<div class="order-form-v2__form-row" style="direction: ltr; padding-top: 25px;">
								<div class="b-form-group">
									...
									<input type="text" id="order_name" name="topic" maxlength="255" data-atest="atest_order_create_form_name" js_default_value="" js_field_code="foc_o_name" class="b-form-group__input" placeholder="Write about…" value="{{ app('request')->input('sw') }}">
...
jQuery.ajax({
 ...
 success: function(ready){
     ...
     if (ready.status == 'old_usr' && ready.msg == usr_email) {
      ...
      $("#signup-title").html("Enter your contact details");
      $("#contacts_button").show();
      $("#get_contact_info").show();
      $("#llogin_button").hide(); 
      $("#llogin_email").hide();
      $(".v7__toggle").hide();
      $(".forgot").hide();
...
error: function(err){
 $("#error_div").html("Something went wrong. Please try again.");
  ...
  $("#contacts_button").click(function(e){
   e.preventDefault();
   var validation = validateContactFields();
   if (validation == false) {
    $("#error_div").html("Please enter correct country and phone number");
    return;
}
...
jQuery.ajax({
 ...
 success: function(response){
     $("#error_div").html(" ");
     if(response.status == "done" && order_session != ''){
      $("#loginModal_1").hide();
      placeOrderFunc();
  }else if (response.status == "done" && order_session == '') {
      $("#loginModal_1").hide();
      $(location).attr('href',url_cu);
  }else{
      $("#error_div").html("Something went wrong. Please try again.");
