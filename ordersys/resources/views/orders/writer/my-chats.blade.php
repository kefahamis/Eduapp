    ...
    <div class="row inner-content_bg_gray">
        ...
        <div class="row custom-row">
            <div class="customer-orders-table__heading">
                <div class="customer-orders-table__title"> Messages</div>
            </div>
                            ...
                            <div class="col-12 col-lg-7 col-xl-9" style="float:right;">
                                ...
                                <div class="flex-grow-0 py-3 px-4 border-top" id="textarea-div">
                                    <div class="input-group">
                                        <textarea class="form-control" id="your_message" placeholder="Your Text Message"></textarea>
...
<script type="text/javascript">
    ...
    $(document).ready(function(){
        $("#textarea-div").hide();
        $(".chat-messages").empty();
        $("#chatting_with").append('<h5>Please select a customer to initiate chat.</h5>');
    ...
    function initChat(recipient){
     ...
     jQuery.ajax({
        url: "{{ url('writer/my-chats/set-recipient') }}",
        method: 'POST',
        data: {
            recipient: recipient,
        },
 ...
 function listRecipients(){
    ...
    jQuery.ajax({
        url: "{{ url('writer/my-chats/list-recipients') }}",
        method: 'POST',
        data: {
            data_str: data_str,
        },
        ...
        success: function(recipients){
           ...
           $.each(recipients, function(index, recipient){
            $("#recipient-div").append("<span class='list-group-item list-group-item-action border-0' onclick=selectRecipient("+recipient.id+")><div class='d-flex align-items-start'><img src='{{asset('images/user avatar.png')}}' class='rounded-circle mr-1' alt='"+recipient.name+"' width='40' height='40'><div class='flex-grow-1 ml-3'>"+recipient.name+"<div class='small'><span class='fas fa-circle chat-online'></span>"+recipient.login_session+"</div></div></div></span>");
        });
       },
       error: function (err) {
        $(".chat-messages").empty();
...
function selectRecipient(recipient){
    ...
    $("#textarea-div").show();
    jQuery.ajax({
        url: "{{ url('writer/my-chats/select-recipient') }}",
        method: 'POST',
        data: {
            recipient: recipient,
        },
...
function sendMessage(recipient,message){
 $("#your_message").val('');
 jQuery.ajax({
    url: "{{ url('writer/my-chats/send-message') }}",
    method: 'POST',
    data: {
        recipient: recipient,
...
function listMessages(recipient){
    ...
    var data_str = "list";
    jQuery.ajax({
        url: "{{ url('writer/my-chats/list-messages') }}",
        method: 'POST',
        data: {
            recipient: recipient,
        },
