...
function get_delivery_address()
{
    ...
    <span style="color:#ff0101;">NOTE:<i>You will require a Sendyit account if you want to use Sendy.</span><span> If you do not have an account <a href="https://app.sendyit.com/auth/sign_up" target="_blank"> click here</a> to create one.</i></span>
...
add_action( 'woocommerce_after_checkout_billing_form', 'get_delivery_address', 10, 0 ); 
...
function getPriceQuote($delivery = false, $type = "quote",  $pick_up_date = null,    $note = "", $recepient_name = "Mutio Samson", $recepient_phone = "0729608407", $recepient_email = "johniemutio@gmail.com")
{
        //if post is set
      ...
      "data": {
        ...
        "from": {
          ...
          "to": {
            ...
            if ($status == false) {
                $str = '';
                $str = "<strong>Sendy API failed!</strong><br>";
                $str .= "<span style=color:red;>".$description."</span><br><br>";
                echo $str;
                return  $result;
            }
        ...
        function displayDelivery( $method )
        {
            if (isset($_POST)) {
                // print_r($method->get_method_id()());
                if( $method->get_method_id() == 'sendy-woocommerce-shipping' ){
                    $to_name = WC()->session->get( 'sendyToName');
 ...
 {   
    ...
    if (!$order_id) {
        ...
        if($orderDetails['status'] === true) {
            ...
            $order_no = $orderDetails['data']['order_no'];
            echo "<p><a target=\"_tab\" href='" . $tracking_link . "'> Click Here To Track Your Sendy Order. </a></p>";
            //save order tracking link
            add_post_meta( $order_id, 'sendy_tracking_link', $tracking_link );
            add_post_meta( $order_id, 'sendy_order_no', $order_no );
